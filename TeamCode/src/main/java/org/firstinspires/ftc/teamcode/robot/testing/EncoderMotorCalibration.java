...
public class EncoderMotorCalibration extends OpMode {
    ...
    double minMotorThreshold = 0; // min motor power so don't burn out motor. Also allows for joysticks to not stall at very low powers if resting position is something like .001

    @Override
    public void init() {
        myEncoderMotor = hardwareMap.dcMotor.get("intake_extender_arm");  //MAP TO YOUR RC CONFIG
//        myEncoderMotor.setDirection(DcMotorSimple.Direction.REVERSE);  // REVERSE AS NEEDED
        ...
        myEncoderMotor.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
